...
Delay:
    clrf    DelayCnt
Delay1:
    incf    DelayCnt	    ; Increment the Counter value
    movf    DelayCnt,w	    ; Put the Counter value in WREG
    goto    $ + 1
    goto    $ + 1
    goto    $ + 1
    goto    $ + 1
    goto    $ + 1
    xorlw   80H		    ; Test the value, set ZERO flag when equal
    skipz		    ; skip is ZERO flag set
    goto    Delay1	    ; Loop when not equal
    retlw   0
;
;   Main
;
